...
int mnist_image_reader::get_rows(){
	return n_rows;
}

int mnist_image_reader::get_cols(){
	return n_cols; 
}

mnist_image_reader::mnist_image_reader(const char* full_path){
...
int mnist_image_reader::set_attributes(){
	int attribute; 
	if(file.is_open()){
...
void mnist_image_reader::read_images(){
...
void mnist_image_reader::pretty_print(int image_number){
	for(int i = 0; i < 28; i++){
		for(int j = 0; j < 28; j++){

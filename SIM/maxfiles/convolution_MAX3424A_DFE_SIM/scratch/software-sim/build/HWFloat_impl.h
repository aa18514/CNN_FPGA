...
long HWFloat<esize,msize>::getValueAsLong() const {
	HWOffsetFix<64,0,TWOSCOMPLEMENT> val_long = cast_float2fixed<64,0,TWOSCOMPLEMENT,TONEAREVEN>(*this);
...
// clearFormating() is needed for compatibility reasons.
template <uint esize, uint msize> inline
HWFloat<esize,msize> HWFloat<esize,msize>::clearFormatting() const {
	return *this;
}
...
CAST_float2fixed(,HWEXCEPT,EXCEPTFLOAT) {
	...
	if (a.getSign() && round_mode != TRUNCATE)
		source = -source;
	...
	if (! (r_offset > a_offset && width_middle > 0) && round_mode != TRUNCATE)
		return HWOffsetFix<r_size, r_offset, r_sign_mode>(result);
	if (! (r_offset > a_offset && width_middle > 0) && round_mode == TRUNCATE)
		return HWOffsetFix<r_size, r_offset, r_sign_mode>(a.getSign() ? -result : result);
	...
	switch(round_mode) {
		...
		case TONEAR :
		round_up = source[cut_bits - 1];
		break;

		case TRUNCATE :
		round_up = false;
		result = a.getSign() ? -result : result;
		break;
	}
	if (round_up)
		result++;

 ...
 *  set_bits method. Single bit access is provided through get_bit, set_bit and
 *  the subscript operator.
 */

#ifndef VARINT64_U_H_
#define VARINT64_U_H_
...
private:
	...
	//static const uint64_t s_mask_low = -1UL;//BIT(s_chunk_bits)-1;
	static const uint64_t s_mask_highest = (size%s_chunk_bits==0) ? -1UL : BIT(size%s_chunk_bits)-1;
...
public:
	...
	int64_t toInt64() const {
		return static_cast<int64_t>(m_store[0]);
	}
	...
	// added to support %s in debug.simPrintf for state machines
	operator std::string() const {
			std::string out;
			bool last_null = false;
			for(int i = s_num_chunks - 1; i >= 0; i--){
				for(int j = 7; j >= 0; j--){
					if(last_null){
						if(get_bits(i * 64 + j * 8, 8) < 32) {
							out.append(1,32);
						} else if(get_bits(i * 64 + j * 8, 8) > 126) {
							out.append(1,32);
						} else {
							out.append(1, get_bits(i * 64 + j * 8, 8));
						}
					} else if(get_bits(i * 64 + j * 8, 8) != 0) {
						last_null = true;
						if(get_bits(i * 64 + j * 8, 8) < 32) {
							out.append(1,32);
						} else if(get_bits(i * 64 + j * 8, 8) > 126) {
	...
	// added to support %c in debug.simPrintf for state machines
	operator char() const {
		if(get_bits(0, 8) < 32) {
			return static_cast<unsigned char>(32);
		} else if(get_bits(0, 8) > 126) {
			return static_cast<unsigned char>(32);
		} else {
			return static_cast<unsigned char>(get_bits(0,8));
		}
	}
	/**
	...
	uint64_t get_bits(int base, int length) const {
		...
		if(l_length == s_chunk_bits) u_word_idx-=1;

		if (u_word_idx > l_word_idx && u_word_idx < s_num_chunks) {	// Upper chunk
				result |= m_store[u_word_idx] << l_length;
		}
	...
	void set_bits(int base, int length, uint64_t value) {
		...
		uint64_t l_mask;
		l_mask  =  ( -1UL << (l_length+l_offset) );
		if(l_mask == -1UL) l_mask = 0;
		l_mask |=  ~( -1UL << (l_offset) );
		const uint64_t l_value = (l_length<64) ? value & ~(-1UL<<l_length) : value;
		m_store[l_word_idx] &= l_mask;
		m_store[l_word_idx] |= (l_value << l_offset);

		if(u_word_idx > l_word_idx && u_word_idx < s_num_chunks) { // has upper
			const uint u_length = (base + length) % s_chunk_bits;
			const uint64_t u_mask = (u_length<64) ? (uint64_t)-1 << u_length : 0;

			m_store[u_word_idx] &= u_mask;
			m_store[u_word_idx] |= (value >> (length-u_length)) & ~u_mask;
		}
		m_store[s_num_chunks-1] &= s_mask_highest;
	}
	...
	varint_u<size>& operator+=(const varint_u<size>& rhs) {
		...
		for(uint i=0; i<s_num_chunks; i++) {
			const uint64_t sum = m_store[i] + rhs.m_store[i] + carry;
			carry = sum<m_store[i] || sum<rhs.m_store[i];
			m_store[i] = sum;
		}
...
/*		uint64_t carry = 1;
	...
	varint_u<size>& operator<<=(int shift_by) {
		...
		if(shift_by == 0) {
			return *this;
		} else if(shift_by >= (int)size) {
			for(uint i=0; i<s_num_chunks; i++) {
				m_store[i] = 0;
			}
			return *this;
		} else if (shift_by < 0) {
			return *this >>= -shift_by;
		}

		const uint elems_shift = shift_by / s_chunk_bits;
		shift_by %= s_chunk_bits;

		if(shift_by != 0) {
			for(uint i=s_num_chunks-1; i>=1+elems_shift; i--) {
				m_store[i] = m_store[i-elems_shift] << shift_by;
				m_store[i] |= m_store[i-1-elems_shift] >> (s_chunk_bits-shift_by);
			}
			m_store[elems_shift] = m_store[0] << shift_by;
		} else {
			for(uint i=s_num_chunks-1; i>=elems_shift; i--) {
				m_store[i] = m_store[i-elems_shift];
			}
		}
	...
	varint_u<size>& operator>>=(int shift_by) {
		...
		if(shift_by != 0) {
			for(uint i=0; i<s_num_chunks-1-elems_shift; i++) {
				m_store[i] = m_store[i+elems_shift] >> shift_by;
				m_store[i] |= m_store[i+1+elems_shift] << (s_chunk_bits-shift_by);
			}
			m_store[s_num_chunks-1-elems_shift] = m_store[s_num_chunks-1] >> shift_by;
		} else {
			for(uint i=0; i<s_num_chunks-elems_shift; i++) {
				m_store[i] = m_store[i+elems_shift];
			}
		}

		for(uint i=s_num_chunks-elems_shift; i<s_num_chunks; i++) {
			m_store[i] = 0;
		}
		...
		varint_u<denom_bits+(frac_bits<0?-frac_bits:0)> *remainder, bool *overflow)
{
	...
	if (hw_exception) {
		...
		*overflow |= lrg_PN[denom_bits+num_bits];
		...
		if (skip_bits - denom_bits > 0) {
			...
			varint_u<front_msb_checked> front = n.template slice<num_bits-front_msb_checked,front_msb_checked>();
			*overflow |= ((bool)front);
		}
	}
	...
	if (get_remainder) {
		...
		*remainder = lrg_PN.template slice<rem_base,rem_bits>();
	}
